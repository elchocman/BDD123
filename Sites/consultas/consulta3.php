...
<?php
  ...
  $titulo = $_POST["titulo"];
 	...
 	$query = "SELECT pelicula.titulo, proveedor.nombre 
      FROM pelicula, proveedor_peliculas, proveedor
      where UPPER(pelicula.titulo) LIKE UPPER('%$titulo%') 
      ...
      where Upper(serie.titulo) LIKE UPPER('%$titulo%') 
      and proveedor_series.id_proveedor = proveedor.id
